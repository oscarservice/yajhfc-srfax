                ...
                ProgressWorker resendWorker = new ProgressWorker() {
                    ...
                    public void doWork() {
                        try {
                            ...
                            ServerOptions so = connection.getOptions();
                            
                            if(so.faxListConnectionType == FaxListConnectionType.SRFAX){
                            	
                            	//SRFAX Resend Implementation
                            	for(FaxJob<? extends FmtItem> job : selectedJobs){
                            		updateNote(infoFormat.format(new Object[] {job.getIDValue()}));
                            		
                            		String filename = job.toString().substring(job.toString().indexOf(":")+1);
                        			String detailsID = filename.substring(filename.indexOf("|")+1);
                        			String fileData = "&sDirection=OUT&sFaxFileName="+filename+"&sFaxDetailsID="+detailsID;
                        			
                        			//get fax number
                        			JsonObject fstatus = SRFaxAPI.sendPost("Get_FaxStatus&sFaxDetailsID="+detailsID, so.user, so.pass.getPassword());
                        			if(fstatus.get("Status").getAsString().compareTo("Failed")==0){
                        				dialogUI.showMessageDialog(fstatus.get("Result").getAsString(), _("SRFax Server Error"), JOptionPane.INFORMATION_MESSAGE);
                        				continue;
                        			}
                        			
                        			//resend fax
                        			JsonObject json = SRFaxAPI.sendPost("Forward_Fax&sCallerID="+so.SRFaxNumber+"&sSenderEmail="+so.SRFaxEmail+"&sFaxType=SINGLE&sToFaxNumber="+fstatus.get("Result").getAsJsonObject().get("ToFaxNumber")+fileData, so.user, so.pass.getPassword());
                        			if(json.get("Status").getAsString().compareTo("Failed")==0){
                        				dialogUI.showMessageDialog(json.get("Result").getAsString(), _("SRFax Server Error"), JOptionPane.INFORMATION_MESSAGE);
                        			}else{
                        				SRFaxSentIds.getInstance().ids.add(so.user+"@"+json.get("Result").getAsString());
                        				//delete after successful resend
                        				job.delete();
                        			}
                            	}
...
//            toolbar.add(actSuspend);
    ...
    public void initialize(boolean adminState, String serverToUse) {
        ...
        this.setTitle(VersionInfo.getAppTitle());
    ...
    protected void setDisconnectedUI() {
        ...
        setActReconnectState(true);
        this.setTitle("Disconnected - " + VersionInfo.getAppTitle());
        if (trayIcon != null) {
            ...
            try {
                ...
                SwingUtilities.invokeLater(new Runnable() {
                    public void run() {
                        ...
                        if(clientManager!=null){
                        	MainWin.this.setTitle(clientManager.getUser() + "@" + currentServer.getOptions().host + (clientManager.isAdminMode() ? " (admin)" : "") + " - " + VersionInfo.getAppTitle());
                        	...
                        	MainWin.this.setTitle(currentServer.getOptions().user + "@" + currentServer.getOptions().host + " - " + VersionInfo.getAppTitle());
                        }
                        if (trayIcon != null) {
